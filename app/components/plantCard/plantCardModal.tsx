...
export default function PlantCardModal(props: any) {
  ...
  const fillRooms = (data: string[]) => {
    return data.map((room) => {
      return <option key={data.indexOf(room)}>{room}</option>;
    });
  };
  ...
  const handleSubmit = (e) => {
    ...
    setFormData({
      ...formData,
      id: `${formData.room.slice(0, 3)}${formData.sciName.slice(
        0,
        3
      )}${formData.nickname.slice(0, 3)}`,
    });
        ...
        <div className="flex flex-wrap -mx-3 mb-6">
          ...
          <div className="w-full md:w-1/3 px-3 mb-5 md:mb-0">
            ...
            <div className="relative">
              <select
                className="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-non focus:bg-white focus:border-gray-500"
                id="grid-room"
                value={formData.room}
                onChange={(e) =>
        ...
        <div className="flex flex-wrap -mx-3 mb-6">
          ...
          <div className="w-full md:w-1/2 px-3 mb-6 md:mb-0">
            ...
            <input
              ...
              id="grid-light"
              type="text"
              value={formData.instructions.light}
              onChange={(e) =>
                setFormData({
                  ...formData,
                  instructions: {
